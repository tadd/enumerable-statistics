...
RSpec.describe Enumerable do
  ...
  describe '#geomean_variance' do
    subject(:geomean_variance) { enum.geomean_variance(&block) }
    let(:enum) { [].each }
    let(:block) { nil }
    ...
    with_enum [3.0, 5.0] do
      context 'with population: nil' do
        subject(:geomean_variance) { enum.geomean_variance(population: nil, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(2.0)
        end
      ...
      context 'with population: false' do
        subject(:geomean_variance) { enum.geomean_variance(population: false, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(2.0)
        end
      ...
      context 'with population: true' do
        subject(:geomean_variance) { enum.geomean_variance(population: true, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(1.0)
        end
  ...
  describe '#geomean_stdev' do
    subject(:geomean_stdev) { enum.geomean_stdev(&block) }
    let(:enum) { [].each }
    let(:block) { nil }
    ...
    with_enum [3.0, 5.0] do
      context 'with population: nil' do
        subject(:geomean_stdev) { enum.geomean_stdev(population: nil, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(Math.sqrt(2.0))
        end
      ...
      context 'with population: false' do
        subject(:geomean_stdev) { enum.geomean_stdev(population: false, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(Math.sqrt(2.0))
        end
      ...
      context 'with population: true' do
        subject(:geomean_stdev) { enum.geomean_stdev(population: true, &block) }
        specify do
          expect(subject[0]).to eq(4.0)
          expect(subject[1]).to eq(1.0)
        end
